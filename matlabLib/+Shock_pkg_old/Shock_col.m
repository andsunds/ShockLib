classdef Shock_col < Shock_pkg_old.Shock_MB & Shock_pkg_old.Shock
...
methods
    function obj = Shock_col(m,Z,n, tau, speed_type,speed, t, nu_star, phimaxmin_in, tol)
        ...
        obj=obj@Shock_pkg_old.Shock(args{:});
...
methods (Static=true, Access=protected)
    ...
    function [Phi_single] = Phi_static(USDS,m,Z,n, phi, phimax, phimin, t,Upsilon, V, tau, tol)
        ...
        import Shock_pkg_old.Shock_col
        if USDS==1 %Upstream
    ...
    function f3 = fjIII_reduced(k, phi, phimax,phimin, t,Upsilon)
        ...
        if Qphisq~=0
            f3=erfc(-Shock_pkg_old.w_approx(1-real(sqrt(k.^2+1)),Upsilon)*srt2).*real(k./sqrt(k.^2+Qphisq));
        else
            f3=erfc(-Shock_pkg_old.w_approx(1-real(sqrt(k.^2+1)),Upsilon)*srt2);
        end
    end
    function f2 = fjII_reduced(k, phi, phimax,phimin, t,Upsilon)
        ...
        Qphisq=(phimax-phi)/phiA;
        if Qphisq>0
            f2=erfc(Shock_pkg_old.w_approx(1-real(sqrt(1-k.^2)),Upsilon)*srt2).*real(k./sqrt(Qphisq-k.^2));
        else
            f2=zeros(size(k));
        end
    end
